...
if __name__ == "__main__":
    ...
    df.to_csv(os.path.join(RESULTS_DIR, "phase3_grid.csv"), index=False)
    heatmaps(df, os.path.join(RESULTS_DIR, "phase3_heatmaps.png"))
    stab = stability(df)
    ...
    with open(os.path.join(RESULTS_DIR, "phase3_report.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")
    print(stab.round(2).to_string())
    print(f"\nBaseline 22/55 SMA percentile: {rank:.0%}")
    print("Report -> results/phase3_report.md")

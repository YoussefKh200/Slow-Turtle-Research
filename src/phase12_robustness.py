...
from backtest import ma, WEEKS_PER_YEAR
from stats import perf, sharpe
from phase11_ftmo import overlay
OUT = os.path.join(os.path.dirname(__file__), "..", "results", "phase12")
...
if __name__ == "__main__":
    ...
    with open(os.path.join(OUT, "PHASE12_REPORT.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")
    print(wf.round(2).to_string(), "\n")
    print(sdf.round(2).to_string(), "\n")
    print(f"DSR = {dsr:.3f}")
    print(pdf.round(2).to_string())
    print("Report -> results/phase12/PHASE12_REPORT.md")
